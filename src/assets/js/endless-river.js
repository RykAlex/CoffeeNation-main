jQuery.fn.endlessRiver = function (settings) {
  ...
  return this.each(function () {
    ...
    if ($line.hasClass("reverse")) {
      var $mask = $line.wrap("<div class='mask reverse'></div>");
    } else {
    ...
    function scrollnews(spazio, tempo) {
      if ($line.hasClass("reverse")) {
        $line.animate({ right: "-=" + spazio }, tempo, "linear", function () {
          $line.children("li:last").prependTo($line);
          $line.css("right", 0);
          currentSpazio = $line.children("li:last").outerWidth(true);
    ...
    if (settings.buttons) {
      ...
      j("body").on("click", "#" + id + " .er-controls > .next", function () {
        if (run) {
          j("#" + id + " .er-controls > .pause").toggleClass(
